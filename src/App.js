...
function App() {
  ...
  useEffect(() => {
    if (videoEnded) {
      gsap.to(videoRef.current, { opacity: 0, duration: 3, ease: 'power1.inOut' });
      gsap.to(logoRef.current, { opacity: 1, height: '12rem', duration: 0, ease: 'power1.inOut' }); // Set height directly without animation
      gsap.to(navbarRef.current, { top: 0, duration: 0.2, ease: 'power1.inOut' }); // Show the navbar
    }
  }, [videoEnded]);
  ...
  return (
    <>
      ...
      <section className={`helloworld relative h-[50vh] sm:h-screen w-full bg-white overflow-y-hidden mx-auto flex items-center justify-center transition-colors duration-2000 `}>
        {!videoEnded && <video src={video} className="w-full h-full object-cover z-[102]" autoPlay muted ref={videoRef}></video>}
        {videoEnded && <img src={logo} ref={logoRef} className={` ${scrolled ? 'h-24' : 'h-24 z-102'}`} alt='Logo' />}
      </section>
